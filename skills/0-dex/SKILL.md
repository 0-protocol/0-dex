...
You can accept their limit order requests and broadcast them to the `gateway.0-protocol.org` relay. Note that settlement is currently simulated while the escrow smart contracts are being audited.

*Note: This skill requires the node gateway and your private key to be configured in `.secrets.env` under `ZERO_DEX_KEY` and `ZERO_DEX_GATEWAY` (Defaults to `mock`).*
